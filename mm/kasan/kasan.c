...
{
	int redzone_adjust;
	cache_size_t new_size = *size + sizeof(struct kasan_alloc_meta);
	int free_meta_offset = 0;

	if (cache->flags & SLAB_DESTROY_BY_RCU || cache->ctor ||
	    cache->object_size < sizeof(struct kasan_free_meta)) {
		free_meta_offset = new_size;
		new_size += sizeof(struct kasan_free_meta);
	}

	redzone_adjust = optimal_redzone(cache->object_size) -
		(new_size - cache->object_size);
	if (redzone_adjust > 0)
		new_size += redzone_adjust;
	/* Make sure the adjusted size is still less than
	 * KMALLOC_MAX_CACHE_SIZE.
	 * TODO: this check is only useful for SLAB, but not SLUB. We'll need
	 * to skip it for SLUB when it starts using kasan_cache_create().
	 */
	if (new_size > KMALLOC_MAX_CACHE_SIZE/2)
		return;
	*flags |= SLAB_KASAN;
	cache->kasan_info.alloc_meta_offset = *size;
	cache->kasan_info.free_meta_offset = free_meta_offset;
	*size = new_size;
}
#endif
